...
class IGFeedPostHeaderTableViewCell: UITableViewCell {
        ...
        private var model: PostViewData?
        ...
        override func layoutSubviews() {
                ...
                postTimeLabel.frame =
                        CGRect(
                                ...
                                width: contentView.width - (size * 2) - 15,
                                height: postTimeLabelSize.height
                        ).integral
        ...
        public func configure(with model: PostViewData) {
                self.model = model
                usernameLabel.text = model.user.username
                //guard let imageHeader = model.user?.profile?.imageHeader else { return }
                //profilePhotoImageView.sd_setImage(with: URL(string: imageHeader), completed: nil)

                let image = model.user.profile?.imageHeader ?? ""
