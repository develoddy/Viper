...
class CommentsPresenter : CommentsPresenterProtocol {
    ...
    var userpostReceivedFromHome: PostViewData?
    private var commentPost: CommentPost?
    private var token = Token()
    
    // MARK: CLOSURES
    //var renderModels: [Comment] = [] {
    var renderModels: [CommentViewData] = [] {
        didSet {
            self.view?.updateUI()
        }
    }
    ...
    // GET DATA COMMENTS.
    func showCommentsData(indexPath: IndexPath) -> CommentViewData /*Comment*/ {
        return self.renderModels[indexPath.row]
    }
    
    // SHOW DATA HEADER.
    func showHeaderCommentData() -> PostViewData {
    ...
    func updateComment(indexPath: IndexPath, content: String) {
        ...
        let postId = self.renderModels[indexPath.row].postId
        let idComment = self.renderModels[indexPath.row].id
    ...
    func deleteRow(indexPath: IndexPath) {
        ...
        let id = self.renderModels[indexPath.row].id
...
extension CommentsPresenter: CommentsInteractorOutputProtocol {
    ...
    func interactorCallBackListComments(with comments: [Comment]) {
        ...
        let formattedItems = comments.map{CommentViewData(info: $0)}
        self.renderModels = formattedItems
    ...
    func interactorCallBackData(with comment: [Comment]) {
        ...
        let formattedItems = comment.map{CommentViewData(info: $0)}
        self.renderModels = formattedItems
        self.view?.stopActivity()
    }
